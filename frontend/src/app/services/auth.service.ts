...
export class AuthService {
  ...
  constructor(
    ...
    this.userRoles$ = this.userRoleSubject$
      .asObservable()
      .pipe(
        distinctUntilChanged(
          (a, b) => a.length === b.length && a.every((role) => b.includes(role))
        )
      );
    ...
    this.userRoles$.subscribe((roles) =>
      console.log('user roles changed to', roles)
    );
  ...
  getAuthProviders(): Observable<OAuth2ProviderDto[]> {
    return this.httpClient
      .get<OAuth2ProviderDto[]>(this.baseUri + '/providers')
      .pipe(
        catchError(
          this.errorHandler.handleError('Could not fetch Login Providers')
        )
      );
  }

  whoAmI(): Observable<WhoAmI> {
    return this.httpClient.get<WhoAmI>(this.baseUri + '/whoami').pipe(
  ...
  register(username: string): Observable<UserRegistration> {
    return this.httpClient
      .post<UserRegistration>(this.globals.backendUri + '/users', {
        username: username,
        description: '',
      })
      .pipe(
  ...
  private getStoredUserRoles(): UserRole[] {
    ...
    if (
      auth.token &&
      this.getTokenExpirationDate(auth.token).valueOf() < new Date().valueOf()
    ) {
    ...
    return auth.whoAmI.admin ? ['USER', 'ADMIN'] : ['USER'];
  }
