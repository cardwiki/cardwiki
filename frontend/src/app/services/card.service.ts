...
export class CardService {
  ...
  createCard(deckId: number, card: CardUpdate): Observable<CardSimple> {
    ...
    return this.httpClient
      .post<CardSimple>(`${this.deckBaseUri}/${deckId}/cards`, dto)
      .pipe(catchError(this.errorHandler.handleError('Could not create Card')));
  }
  ...
  getCardsByDeckId(
    deckId: number,
    pageable: Pageable
  ): Observable<Page<CardSimple>> {
    console.log('get cards for deck with id ' + deckId);
    return this.httpClient
      .get<Page<CardSimple>>(`${this.deckBaseUri}/${deckId}/cards`, {
        params: pageable.toHttpParams(),
      })
      .pipe(catchError(this.errorHandler.handleError('Could not fetch Cards')));
  }
  ...
  removeCard(cardId: number, message?: string): Observable<void> {
    ...
    return this.httpClient
      .post<void>(`${this.cardBaseUri}/${cardId}`, { message: message })
      .pipe(catchError(this.errorHandler.handleError('Could not remove Card')));
  }
  ...
  editCard(cardId: number, card: CardUpdate): Observable<CardSimple> {
    ...
    const dto = this.toCardUpdateDto(card);
    return this.httpClient
      .patch<CardSimple>(`${this.cardBaseUri}/${cardId}`, dto)
      .pipe(catchError(this.errorHandler.handleError('Could not edit Card')));
  }

  fetchCard(cardId: number): Observable<CardUpdate> {
    console.log(`fetch card with id ${cardId}`);
    return this.httpClient
      .get<CardUpdate>(`${this.cardBaseUri}/${cardId}`)
      .pipe(catchError(this.errorHandler.handleError('Could not fetch Card')));
  }
  ...
  deleteCard(cardId: number): Observable<void> {
    ...
    return this.httpClient
      .delete<void>(`${this.cardBaseUri}/${cardId}`)
      .pipe(catchError(this.errorHandler.handleError('Could not delete Card')));
  }
  ...
  fetchRevisionsById(
    revisionIds: number[]
  ): Observable<{ [key: number]: RevisionDetailed }> {
    console.log(`fetch revisions ${revisionIds}`);
    return this.httpClient
      .get<{ [key: number]: RevisionDetailed }>(
        `${this.globals.backendUri}/revisions/byid?${revisionIds
          .map((id) => `id=${id}`)
          .join('&')}`
      )
      .pipe(
        catchError(this.errorHandler.handleError('Could not fetch Revisions'))
      );
  }
  ...
  fetchRevisions(
    ...
    return this.httpClient
      .get<Page<RevisionDetailed>>(`${this.cardBaseUri}/${cardId}/revisions`, {
        params: pageable.toHttpParams(),
      })
      .pipe(
        catchError(this.errorHandler.handleError('Could not fetch Revisions'))
  ...
  private toCardUpdateDto(card: CardUpdate): CardUpdateDto {
    const cardUpdateDto = new CardUpdateDto(
      card.textFront,
      card.textBack,
      null,
      null,
