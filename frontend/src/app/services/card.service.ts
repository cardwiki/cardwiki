...
export class CardService {
   ...
   */
  fetchRevisionsById(revisionIds: number[]): Observable<Page<RevisionDetailed>> {
    console.log(`fetch revisions ${revisionIds}`);
    return this.httpClient.get<Page<RevisionDetailed>>(`${this.globals.backendUri}/revisions?${revisionIds.map(id => `id=${id}`).join("&")}`)
