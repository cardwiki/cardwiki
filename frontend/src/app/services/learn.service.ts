...
export class LearnService {
  ...
  getNextCards(
    ...
    return this.httpClient
      .get<CardSimple[]>(this.learnBaseUri + '/next', { params })
      .pipe(
        catchError(
          this.errorHandler.handleError('Could not fetch next learning cards')
        )
      );
  }
  ...
  sendAttemptStatus(learnAttempt: LearnAttempt) {
    console.log(
      'Sending status for most recent attempt for Card with id ' +
        learnAttempt.cardId
    );
    return this.httpClient
      .post<LearnAttempt>(this.learnBaseUri + '/attempt', learnAttempt)
      .pipe(
        catchError(
          this.errorHandler.handleError('Could not save learning attempt')
        )
      );
  }
}
