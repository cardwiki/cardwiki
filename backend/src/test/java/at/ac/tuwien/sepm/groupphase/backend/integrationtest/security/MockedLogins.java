...
public abstract class MockedLogins {
    ...
    public static SecurityMockMvcRequestPostProcessors.OAuth2LoginRequestPostProcessor mockLogin(Collection<? extends GrantedAuthority> authorities, String oAuthId){
        OAuth2User user = Mockito.mock(OAuth2User.class);
        Mockito.doReturn(authorities).when(user).getAuthorities();
        Mockito.doReturn(oAuthId).when(user).getName();
